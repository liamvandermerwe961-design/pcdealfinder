...
(function () {
  ...
  function productByName(name) {
    ...
    return (window.products || []).find(p => String(p.name || '').trim().toLowerCase() === target) || null;
  }
  ...
  function saveBuild(state) {
    try {
      localStorage.setItem('pcdf-saved-build', JSON.stringify(state));
      return true;
  ...
  function shareBuild(state) {
    ...
    if (navigator.clipboard) {
      navigator.clipboard.writeText(url).then(() => toast('Build link copied ✓')).catch(() => prompt('Copy your build link:', url));
    } else {
      prompt('Copy your build link:', url);
    }
  }
  ...
  function compatibilityChecks() {
    ...
    if (cpu && motherboard && cpu.socket && motherboard.socket) {
      checks.push({ ok: cpu.socket === motherboard.socket, title: 'CPU ↔ motherboard', detail: cpu.socket === motherboard.socket ? cpu.socket + ' socket matched' : cpu.socket + ' CPU vs ' + motherboard.socket + ' motherboard' });
    }
    if (ram && motherboard && ram.memory && motherboard.memory) {
      checks.push({ ok: ram.memory === motherboard.memory, title: 'RAM ↔ motherboard', detail: ram.memory === motherboard.memory ? ram.memory + ' memory matched' : ram.memory + ' RAM vs ' + motherboard.memory + ' motherboard' });
    }
    if (gpu && psu) {
      const gpuPower = Number(gpu.wattage) || estimateGpuPower(gpu);
      const psuPower = Number(psu.wattage) || 0;
      const required = gpuPower + 200;
  ...
  function enhanceResult() {
    ...
    panel.innerHTML = `
      <div class="phase1Head">
        <div><small>BUILD HEALTH</small><h4>${failures.length ? 'Compatibility needs attention' : 'Compatible build'}</h4></div>
        <span>${failures.length ? '⚠ Review the warning below' : '✓ Core compatibility checks passed'}</span>
      </div>
      <div class="compatibilityGrid">
        ${checks.length ? checks.map(c => `<div class="compatibilityItem ${c.ok ? 'ok' : 'bad'}"><b>${c.ok ? '✓' : '⚠'} ${c.title}</b><span>${c.detail}</span></div>`).join('') : '<div class="compatibilityItem ok"><b>✓ Catalogue compatibility</b><span>No conflicting specifications were found.</span></div>'}
      </div>
      <div class="phase1Actions">
        <button type="button" class="builderSecondary" id="saveCurrentBuild">♡ Save build</button>
        <button type="button" class="builderSecondary" id="shareCurrentBuild">↗ Share build</button>
      </div>`;
    result.appendChild(panel);

    const state = getBuildState();
    panel.querySelector('#saveCurrentBuild')?.addEventListener('click', () => {
      saveBuild(state);
  ...
  function wrap() {
    ...
    window.__pcdfPhase1Wrapped = true;
  ...
  function loadSharedBuild() {
    ...
    try {
      ...
      set('buildBudget', state.budget);
      set('buildGame', state.game);
      set('buildResolution', state.resolution);
      set('buildPriority', state.priority);
      setTimeout(() => { if (typeof window.generateBuild === 'function') window.generateBuild(); }, 300);
    } catch (_) {}
  }
